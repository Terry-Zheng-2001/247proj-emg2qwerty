...
class TokenEmbedding(nn.Module):
    def __init__(self, c_in, d_model):
        ...
        self.conv2 = nn.Conv1d(in_channels=d_model//2, out_channels=d_model//2,
                               kernel_size=3, padding=1, padding_mode='circular', bias=False)
        self.conv3 = nn.Conv1d(in_channels=d_model//2, out_channels=d_model,
                               kernel_size=3, padding=1, padding_mode='circular', bias=False)
        ...
        # 1x1 Conv 作为 residual connection (如果 c_in != d_model，需要调整维度)
        self.residual_conv = nn.Conv1d(in_channels=c_in, out_channels=d_model,
                                       kernel_size=1, bias=False) if c_in != d_model else nn.Identity()
    ...
    def forward(self, x):
        ...
        residual = self.residual_conv(x)  # 处理 residual 路径
